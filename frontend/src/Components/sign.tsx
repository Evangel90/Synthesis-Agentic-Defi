  ...
  accounts: {
    oauth2: {
      initTokenClient: (config: {
        client_id: string;
        scope: string;
        callback: (response: GoogleAccessTokenResponse) => void;
...
export function SignIn() {
  ...
  useEffect(() => {
      ...
      googleTokenClient.current = window.google.accounts.oauth2.initTokenClient(
        {
            ...
            try {
              ...
              setGoogleError(
                error instanceof Error
                  ? error.message
                  : "Google sign-in completed, but profile loading failed.",
              );
            } finally {
              setGoogleLoading(false);
            }
          ...
          error_callback: ({ type }) => {
            setGoogleError(`Google sign-in could not start: ${type}.`);
            setGoogleLoading(false);
          },
        },
      );
    ...
    const handleLoad = () => initializeGoogleClient();
    const handleError = () => {
  ...
  const googleStatus = useMemo(() => {
    ...
    if (!googleClientId) {
      return "Add VITE_GOOGLE_CLIENT_ID to frontend/.env.local to enable Google sign-in.";
    }
    ...
    return "";
  }, [googleClientId, googleError, googleLoading, googleProfile, googleReady]);
  ...
  const handleGoogleSignIn = () => {
    if (!googleClientId) {
      setGoogleError(
        "Missing VITE_GOOGLE_CLIENT_ID. Add it to frontend/.env.local first.",
      );
      return;
    }
